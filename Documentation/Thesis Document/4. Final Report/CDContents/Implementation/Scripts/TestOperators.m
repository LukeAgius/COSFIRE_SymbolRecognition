...
display(' ')
display('=========== 3. CLASSIFICATION STAGE ===========')
    ...
    for i=0:length(tests)-1,   

        % Init testing image
       testingImage = imread([Utilities.getTestingDirectory(params) 'file_' num2str(i) params.TestingExtension]);
...
%         testingImage = imcomplement(imdilate(S,strel('disk',2)));
       ...
       % if the most maximum value within the testingImage is equal to 255
        ...
        % for user to file.
%         if strrep(models(index).name,params.ModelsExtension,'') == Utilities.ReadXMLV1(['file_' num2str(i)])
%             % Increment true positives
%             truepositives = truepositives + 1;
%             % Save result to file
%             Utilities.appendResultToFile(params.TestingFolder,['file_' num2str(i) params.TestingExtension],models(index).name,'Pass');
%         else
%             % Increment false positives
%             falsepositives = falsepositives + 1;            
%             % Save result to file
%             Utilities.appendResultToFile(params.TestingFolder,['file_' num2str(i) params.TestingExtension],models(index).name,'Fail');
%             % Save false positive to image.
%             % Utilities.saveFalsePositive(params.TestingFolder, testingImage, imread([Utilities.getModelsDirectory(params) models(index).name]), ['file_' num2str(i) params.TestingExtension]);
%         end
        
        
        if strrep(models(index).name,params.ModelsExtension,'') == Utilities.ReadXMLV1([Utilities.getTestingDirectory(params) 'file_' num2str(i) '.xml'])
            % Increment true positives & safe result to file
            truepositives = truepositives + 1;
            Utilities.appendResultToFile(params.TestingFolder,['file_' num2str(i) params.TestingExtension],models(index).name,'Pass');
        else
            % Increment false positives & safe result to file
            falsepositives = falsepositives + 1;            
            Utilities.appendResultToFile(params.TestingFolder,['file_' num2str(i) params.TestingExtension],models(index).name,'Fail');
        end
    end
