function [output, tuple] = applyCOSFIRE(inputImage,operator, tuple)
...
% params.inputfilter.Gabor.inhibition.method = 3;
% params.inputfilter.Gabor.inhibition.alpha = 1;
% u=unique(inputfilterparams(:,1));
% for i = 1:length(u)
%     f=find(inputfilterparams(:,1) == u(i));
%     GaborOutput = getGaborResponse(inputImage,params.inputfilter.Gabor,u(i),inputfilterparams(f,2));    
%     for j = 1:length(f)
%         hashkeylist{f(j)} = getHashkey(inputfilterparams(f(j),:));
%         hashvaluelist{f(j)} = f(j);                        
%         inputfilterresponse(:,:,f(j)) = GaborOutput(:,:,j);
%     end
% end
